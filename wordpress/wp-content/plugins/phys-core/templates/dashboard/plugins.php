...
$plugins      = Phys_Plugins_Manager::get_plugins();
$add_ons      = Phys_Plugins_Manager::get_all_add_ons();
$writable     = Phys_Plugins_Manager::get_permission();
$default_desc = phys_core_get_content_json_url( Phys_Admin_Config::get( 'host_downloads_api' ) . 'desc-plugin.json' );
...
<div class="wrap plugin-tab">
	...
	<h2 class="screen-reader-text"><?php esc_html_e( 'Filter plugins list', 'phys-core' ); ?></h2>
	<div class="wp-filter">
		<ul class="filter-links">
			<li class="all" data-filter="*"><a href="#all" class="current"><?php esc_html_e( 'All ', 'phys-core' ); ?>
					(<span>__</span>)</a></li>
			<li class="required" data-filter=".required"><a href="#required"
															class=""><?php esc_html_e( 'Required ', 'phys-core' ); ?>
					(<span>__</span>)</a></li>
			<li class="recommended" data-filter=".recommended"><a
					href="#recommended"><?php esc_html_e( 'Recommended', 'phys-core' ); ?>(<span>__</span>)</a></li>
			<?php if ( count( $add_ons ) ) : ?>
				<li class="add-ons" data-filter=".add-on"><a
						href="#add-ons"><?php printf( __( 'Add-ons (%s)', 'phys-core' ), count( $add_ons ) ); ?></a>
				</li>
			<?php endif; ?>
			<li class="updates" data-filter=".can-update"><a
					href="#updates"><?php esc_html_e( 'Update Available ', 'phys-core' ); ?>(<span>__</span>)</a>
			</li>
		</ul>

		<div class="search-form search-plugins">
			<input type="hidden" name="tab" value="search">
			<label><span class="screen-reader-text"><?php esc_html_e( 'Search plugins', 'phys-core' ); ?></span>
				<input type="search" name="s" value="" class="wp-filter-search"
					   placeholder="<?php esc_attr_e( 'Search plugins', 'phys-core' ); ?>"
					   aria-describedby="live-search-desc">
			</label>
			<input type="submit" id="search-submit" class="button hide-if-js"
				   value="<?php esc_attr_e( 'Search Plugins', 'phys-core' ); ?>"></div>
	</div>
	<br class="clear">
	...
	<div id="plugin-filter">
		<div class="list-plugins">
			...
			foreach ( $plugins as $index => $plugin ) :
				...
				$plugin_icon = PHYS_CORE_ADMIN_URI . '/assets/images/logo.svg';
				if ( $plugin->get_icon() ) {
				...
				$plugin_icon = apply_filters( 'phys_core_plugin_icon_install', $plugin_icon, $plugin );
				...
				<div class="plugin-card <?php echo esc_attr( $plugin_classes ); ?>"
								...
								<li>
									<?php if ( $can_update ) : ?>
										<button type="button" class="button" data-premium="<?php echo $premium; ?>"
												data-action="update" <?php disabled( $writable, false ); ?> ><?php esc_html_e( 'Update', 'phys-core' ); ?></button>
									<?php elseif ( $status == 'not_installed' ) : ?>
										<button type="button" class="button" data-premium="<?php echo $premium; ?>"
												data-action="install" <?php disabled( $writable, false ); ?> ><?php esc_html_e( 'Install Now', 'phys-core' ); ?></button>
									<?php elseif ( $status == 'inactive' ) : ?>
										<button type="button" class="button"
												data-action="activate"><?php esc_html_e( 'Activate', 'phys-core' ); ?></button>
									<?php elseif ( $status == 'can_installed' ) : ?>
										<button type="button" class="button button-disabled"
												disabled><?php esc_html_e( 'Cannot Install', 'phys-core' ); ?></button>
									<?php else : ?>
										<button type="button" class="button"
											<?php disabled( true, $disable_deactivate, true ); ?>
												data-action="deactivate"><?php esc_html_e( 'Deactivate', 'phys-core' ); ?></button>
									<?php endif; ?>
								</li>
							</ul>
						</div>
					...
					<?php if ( $version || $plugin->is_required() ) : ?>
						<div class="plugin-card-bottom">
							<?php if ( $version ): ?>
								<div class="column-downloaded"><?php echo __( 'Version: ', 'phys-core' ); ?><span
										class="data_Version"><?php echo esc_html( $version ); ?></span></div>
							<?php endif; ?>
							<?php if ( $plugin->is_required() ) : ?>
								<div class="column-updated"><span
										class="plugin-required"><?php esc_html_e( 'Required', 'phys-core' ); ?></span>
								</div>
							<?php endif; ?>
						</div>
					<?php endif; ?>

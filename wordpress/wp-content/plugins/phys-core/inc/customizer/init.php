...
class Init {
	...
	public function hooks() {
		...
		/**
		 * When use Kirki can't set variants mutilple so in theme use 'phys_multiple_variants_fonts' for select variants.
		 * Use this hook to set when customer update phys core use new customizer.
		 *
		 * TODO: Remove this code when Customer use this customizer.
		 */
		add_filter(
			'phys_customizer_typography_family_variants_default',
			function( $variants, $id ) {
				$variant_fallback = get_theme_mod( 'phys_multiple_variants_fonts', array() );

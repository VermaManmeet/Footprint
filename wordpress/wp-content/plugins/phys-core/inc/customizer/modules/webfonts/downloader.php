<?php
namespace PhysCode\Customizer\Modules\Webfonts;
...
class Downloader {
	...
	protected $font_folder = 'phys-fonts';
	...
	public function download_fonts_to_local() {
		...
		$stored = get_option( 'phys_customizer_downloaded_ggfont_files', array() );
		$stored_names = array();
		$change     = false;
		...
		if ( $change ) {
			update_option( 'phys_customizer_downloaded_ggfont_files', $stored );
		}
	...
	public function get_content_cache() {
		$cache = get_site_transient( 'phys_customizer_ggfonts_content_cache' );
		...
		if ( $contents ) {
			...
			set_site_transient( 'phys_customizer_ggfonts_content_cache', $cache, 60 * 60 * 24 * 7 );
		}

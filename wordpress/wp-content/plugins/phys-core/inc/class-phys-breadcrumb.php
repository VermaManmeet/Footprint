<?php
/**
 * Phys_Breadcrumbs class.
 *
 */

namespace PhysCoreBreadcrumbs;

if ( ! class_exists( 'Phys_Breadcrumbs' ) ) {
	class Phys_Breadcrumbs {
		/**
		 * @var array
		 */
		public $data = array();
		...
		public function breadcrumb_structured_data( $crumbs ) {
			...
			$this->data[] = apply_filters( 'phys/structured_data/breadcrumblist', $markup, $crumbs );
		}
		...
		public function get_structured_data() {
			...
			$types = apply_filters( 'phys/structured_data/types', array( 'breadcrumblist' ) );
		 ...
		 * @return array
		 */
		public function phys_get_breadcrumb() {
			return apply_filters( 'phys_get_breadcrumb', $this->crumbs, $this );
		}
		...
		public function render() {
			...
			$text_home = apply_filters( 'phys_breadcrumb_text_home', _x( 'Home', 'breadcrumb', 'phys-core' ) );
			...
			if ( ! is_front_page() || is_paged() ) {
				...
				// Structured
				$this->breadcrumb_structured_data( $this->phys_get_breadcrumb() );

				return $this->phys_get_breadcrumb();
			}
		...
		protected function add_crumbs_404() {
			$this->add_crumb( __( 'Error 404', 'phys-core' ) );
		}
		...
		protected function add_crumbs_search() {
			if ( is_search() ) {
				$this->add_crumb( sprintf( esc_html__( 'Search results for &ldquo;%s&rdquo;', 'phys-core' ), get_search_query() ), remove_query_arg( 'paged' ) );
			}
		}
		...
		protected function add_crumbs_current_archive_single( $post_id = '' ) {
			if ( get_post_type() == 'lp_course' ) {
				...
				$taxonomy = apply_filters( 'phys_breadcrumb_taxonomy_single_post', '' );
			}
			...
			if ( $terms ) {
				$main_term = apply_filters( 'phys_breadcrumb_main_term', $terms[0], $terms );
		...
		protected function add_crumbs_post_type_archive() {
			...
			if ( $post_type ) {
				if ( get_post_type() == 'lp_course' ) {
					...
					$custom_name = apply_filters( 'phys_breadcrumb_post_type_name', '' );
				}
		...
		protected function add_crumbs_author() {
			...
			$this->add_crumb( sprintf( __( 'Author: %s', 'phys-core' ), $userdata->display_name ) );
		}
		...
		protected function crumb_paged() {
			if ( get_query_var( 'paged' ) ) {
				$this->add_crumb( sprintf( esc_html__( 'Page %d', 'phys-core' ), get_query_var( 'paged' ) ) );
			}
		}
	}
}

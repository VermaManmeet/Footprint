...
if ( ! class_exists( 'Phys_Plugin' ) ) {
	class Phys_Plugin {
		...
		public function get_source() {
			...
			return apply_filters( 'phys_core_url_download_private_plugin', $source, $this );
		}
		...
		public function activate( $silent = null, $network_wide = false, $plugin_file_relative = '' ) {
			...
			do_action( 'phys_core_activate_plugin', $plugin, $network_wide, $silent );
		...
		public function install_by_local_file() {
			$file_path = $this->download_from_physpress();
		...
		public function wp_install( $package ) {
			do_action( 'phys_core_pre_install_plugin', $package, $this );
		...
		public function update() {
			try {
				...
				if ( ! $this->is_wporg ) {
					$plugin = $this->download_from_physpress();
				...
				return new WP_Error( 'phys_core_update_plugin', $e->getMessage() );
			}
		}

		/**
		 * Download addon from PhysCode.
		 *
		 * return string|WP_Error
		 */
		public function download_from_physpress() {
...
//				return new WP_Error( 'phys_err_download_plugin', $data );
//			}
			// New add
			...
			if ( ! empty( $data_json['message'] ) ) {
				...
				return new WP_Error( 'phys_err_download_plugin', ! empty( $data_json['message'] ) ? $data_json['message'] : __( 'Error download plugin', 'phys-core' ) );
			}

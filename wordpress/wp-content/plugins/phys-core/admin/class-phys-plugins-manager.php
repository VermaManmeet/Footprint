...
/**
 * Class Phys_Plugins_Manager.
 *
 * @since 0.5.0
 */
class Phys_Plugins_Manager extends Phys_Admin_Sub_Page {
	/**
	 * @since 1.1.0
	 *
	 * @var null
	...
	public static function add_notification( $content = '', $type = 'success' ) {
		Phys_Dashboard::add_notification(
			array(
				'content' => $content,
				'type'    => $type,
				'page'    => self::$page_key,
	...
	/**
	 * Phys_Plugins_Manager constructor.
	 *
	 * @since 0.4.0
	 */
	...
	public static function get_link_download_plugin( $slug ) {
		$url = '';
 		$purchase_token = Phys_Product_Registration::get_data_theme_register('purchase_token');
 		if ( $purchase_token ) {
			$url = Phys_Admin_Config::get( 'host_downloads_api' ) . '/wp-json/phys-market/v1/download-plugin/';
			$url = add_query_arg(
				array(
					'plugin'    => $slug,
		...
		return apply_filters( 'phys_core_get_link_download_plugin', $url );
	}
	...
	private function init_hooks() {
		add_action( 'wp_ajax_phys_plugins_manager', array( $this, 'handle_ajax_plugin_request_actions' ) );
		add_action( 'admin_enqueue_scripts', array( $this, 'enqueue_scripts' ) );
		add_filter( 'phys_dashboard_sub_pages', array( $this, 'add_sub_page' ) );
		add_action( 'admin_action_phys_plugins', array( $this, 'handle_plugin_actions' ) );
		add_action( 'phys_core_dashboard_init', array( $this, 'notification' ) );
		add_filter( 'phys_core_url_download_private_plugin', array( $this, 'filter_link_download_plugin' ), 10, 2 );
		add_action( 'phys_core_pre_install_plugin', array( $this, 'raise_limit_plugin_installation' ) );
		add_action( 'phys_core_pre_upgrade_plugin', array( $this, 'raise_limit_plugin_installation' ) );
	}
	...
	public function raise_limit_plugin_installation() {
		...
		wp_raise_memory_limit( 'phys_core_admin' );
	}
	...
	public function filter_link_download_plugin( $source, $plugin ) {
		//        if ( ! Phys_Product_Registration::is_active() && ! Phys_Free_Theme::is_free() ) {
	...
	public function handle_plugin_actions() {
		...
		$plugin = new Phys_Plugin( $slug, $is_wporg );
		...
		if ( $action == 'install' ) {
			...
			phys_core_redirect( $link_activate );
		}
		...
		phys_core_redirect( admin_url( 'plugins.php' ) );
	}
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( 'phys-isotope', PHYS_CORE_ADMIN_URI . '/assets/js/plugins/isotope.pkgd.min.js', array( 'jquery' ), PHYS_CORE_VERSION );
		wp_enqueue_script(
			'phys-plugins-manager',
			PHYS_CORE_ADMIN_URI . '/assets/js/plugins/plugins-manager.min.js',
			array(
				'phys-plugins',
				'phys-isotope',
				'updates',
	...
	public static function get_plugins() {
		if ( self::$plugins == null ) {
			...
			foreach ( $all_plugins as $index => $plugin ) {
				$phys_plugin = new Phys_Plugin();
				$phys_plugin->set_args( $plugin );
	...
	private static function get_all_plugins() {
		if ( self::$all_plugins_require == null ) {
			...
			$plugins = apply_filters( 'phys_core_get_all_plugins_require', $plugins );
			...
			self::$all_plugins_require = apply_filters( 'phys_core_list_plugins_required', $plugins );
		}
	...
	public static function get_external_plugins() {
		...
		$plugins = array();
		foreach ( $required_plugins as $phys_plugin ) {
			if ( $phys_plugin->is_wporg() ) {
				continue;
			}
	...
	public static function get_required_plugins_inactive() {
		...
		foreach ( $required_plugins as $phys_plugin ) {
			...
			if ( ($phys_plugin->is_required() == false && in_array( $phys_plugin->get_slug(), $no_install )) ||   $phys_plugin->no_install_default() ) {
				$install_pl = true;
			}
			if ( $install_pl || $phys_plugin->is_active() || $phys_plugin->is_add_on() ) {
				continue;
			}

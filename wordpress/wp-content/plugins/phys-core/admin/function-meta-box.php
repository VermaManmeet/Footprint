 ...
 * @param array $field
 */
function phys_meta_box_text_input_field( $field ) {
	...
	$field['type']          = ( isset( $field['type'] ) && $field['type'] != 'textfield' ) ? $field['type'] : 'text';
	...
	echo '<div class="inner-form-field">';
 ...
 * @param array $field
 */
function phys_meta_box_textarea_field( $field ) {
	global $thepostid, $post;
 ...
 * @param array $field
 */
function phys_meta_box_checkbox_field( $field ) {
	global $thepostid, $post;
 ...
 */
function phys_meta_box_select_field( $field ) {
	global $thepostid, $post;
	...
	$field = wp_parse_args(
		...
		array(
			'class'             => 'select',
			'style'             => '',
			'wrapper_class'     => '', // Use "phys-select-2" for select2.
	...
	<div class="form-field <?php echo esc_attr( $field['id'] ) . '_field ' . esc_attr( $field['wrapper_class'] ); ?>">
		...
		<select <?php echo phys_implode_html_attributes( $field_attributes ); ?>>
			<?php
 ...
 * @param array $field
 */
function phys_meta_box_radio_field( $field ) {
	global $thepostid, $post;
	...
	echo '<ul class="phys-radios-field-meta-box">';
...
function phys_meta_box_file_input_field( $field ) {
	global $thepostid, $post;
	...
	echo '<div id="' . esc_attr( $field['id'] ) . '" class="phys-meta-box__file ' . esc_attr( $field['class'] ) . '" data-mime="' . $field['mime_type'] . '" data-multil="' . $field['multil'] . '" style="' . esc_attr( $field['style'] ) . '" ' . implode( ' ', $custom_attributes ) . '>';
	echo '<ul class="phys-meta-box__file_list">';
	...
	if ( ! empty( $field['value'] ) ) {
		foreach ( (array) $field['value'] as $attachment_id ) {
			...
			if ( $url ) {
				...
				echo '<li class="phys-meta-box__file_list-item image" data-attachment_id="' . $attachment_id . '">';
	...
	echo '</ul>';
	echo '<input class="phys-meta-box__file_input" type="hidden" name="' . esc_attr( $field['id'] ) . '" value="' . esc_attr( ( ! empty( $field['value'] ) && is_array( $field['value'] ) ) ? implode( ',', $field['value'] ) : $field['value'] ) . '" />';
	echo '<div>';
 ...
 * @param array $field
 */
function phys_meta_box_duration_field( $field ) {
	global $thepostid, $post;
	...
	echo '<div class="phys-meta-box__duration form-field ' . esc_attr( $field['id'] ) . '_field ' . esc_attr( $field['wrapper_class'] ) . '" data-hide="' . esc_attr( ! empty( $field['hidden'] ) ? htmlentities( wp_json_encode( $field['hidden'] ) ) : '' ) . '">
		<label for="' . esc_attr( $field['id'] ) . '">' . wp_kses_post( $field['label'] ) . '</label>';
	...
	echo '<select name="' . esc_attr( $field['name'] ) . '[]" class="phys-meta-box__duration-select">' . $html_option . '</select>';
...
function phys_meta_box_color_input_field( $field ) {
	global $thepostid, $post;
	...
	wp_enqueue_script( 'phys-wp-color-picker-alpha' );

	echo '<div class="form-field phys-meta-box__color ' . esc_attr( $field['id'] ) . '_field ' . esc_attr( $field['wrapper_class'] ) . '" data-hide="' . esc_attr( ! empty( $field['hidden'] ) ? htmlentities( wp_json_encode( $field['hidden'] ) ) : '' ) . '">
		<label for="' . esc_attr( $field['id'] ) . '">' . wp_kses_post( $field['label'] ) . '</label>';

	echo '<input type="text" class="phys-meta-box__color--input ' . esc_attr( $field['class'] ) . '" style="' . esc_attr( $field['style'] ) . '" name="' . esc_attr( $field['name'] ) . '" id="' . esc_attr( $field['id'] ) . '" value="' . esc_attr( $field['value'] ) . '" placeholder="' . esc_attr( $field['placeholder'] ) . '" dir="ltr" ' . implode( ' ', $custom_attributes ) . ' /> ';
...
function phys_meta_box_image_select_field( $field ) {
	global $thepostid, $post;
	...
	if ( $field['options'] ) {
		echo '<div class="phys-meta-box__image_select">';
		foreach ( $field['options'] as $key => $img ) {
			echo '<label class="phys-meta-box__image_select--radio ' . esc_attr( $field['class'] ) . '" style="' . esc_attr( $field['style'] ) . '" ' . implode( ' ', $custom_attributes ) . '>';
...
function phys_implode_html_attributes( $raw_attributes ) {
	$attributes = array();
 ...
 * @param array $field
 */
function phys_meta_box_wysiwyg_field( $field ) {

	global $thepostid, $post;

	$thepostid              = empty( $thepostid ) ? $post->ID : $thepostid;
	$field['wrapper_class'] = isset( $field['wrapper_class'] ) ? $field['wrapper_class'] : 'phys-wysiwyg-wrapper';
	$field['default']       = ( ! get_post_meta( $thepostid, $field['id'], true ) && isset( $field['default'] ) ) ? $field['default'] : get_post_meta( $thepostid, $field['id'], true );
	$field['value']         = isset( $field['value'] ) && $field['value'] !== false ? $field['value'] : $field['default'];
	$field['name']          = isset( $field['name'] ) ? $field['name'] : $field['id'];
...
function phys_meta_box_heading_field( $field ) {
	global $thepostid, $post;
	$thepostid              = empty( $thepostid ) ? $post->ID : $thepostid;
	$field['class']         = isset( $field['class'] ) ? $field['class'] : 'select';
	$field['style']         = isset( $field['style'] ) ? $field['style'] : '';
	...
	echo $field['before'];
	echo '<div class="form-field form-heading-wrapper ' . esc_attr( $field['id'] ) . '_field ' . esc_attr( $field['wrapper_class'] ) . '" data-hide="' . esc_attr( ! empty( $field['hidden'] ) ? htmlentities( wp_json_encode( $field['hidden'] ) ) : '' ) . '">' . wp_kses_post( $field['label'] ) ;
	echo '</div>';
	echo $field['after'];
}

...
/**
 * Class Phys_Self_Update
 *
 * @since 1.0.4
 */
class Phys_Self_Update extends Phys_Singleton {
	/**
	 * @since 1.0.4
	 *
	 * @var string
	 */
	private $slug = 'phys-core';
	 ...
	 * @var string
	 */
	private $plugin = 'phys-core/phys-core.php';
	 ...
	 * @since 1.0.5
	 */
	private $url_changelog = 'https://physpresswp.github.io/phys-core/changelog.html';

	/**
	 * Phys_Self_Update constructor.
	 *
	 * @since 1.0.4
	 */
	...
	private function init_hooks() {
		...
		add_filter( 'phys_core_list_plugins_required', array( $this, 'add_phys_core_plugin' ) );
	}

	/**
	 * Add Phys Core package to list plugins.
	 *
	 * @param $plugins
	 *
	 * @return array
	 ...
	 *
	 */
	public function add_phys_core_plugin( $plugins ) {
		$plugins[] = array(
			'name'               => 'Phys Core',
			'slug'               => 'phys-core',
 			'premium'            => true,
	...
	public function filter_plugin_information( $false, $action, $args ) {
		...
		if ( isset( $args->slug ) && $args->slug === $this->slug ) {
			...
			$information->name                    = __( 'Phys Core by PhysCode', 'phys-core' );
	...
	private function get_changelog() {
		...
		if ( $code != 200 ) {
			return __( 'Sorry! Something went wrong. Changelog could not be loaded.', 'phys-core' );
		}
	...
	/**
	 * Get check update plugin phys core (from GitHub.io).
	 *
	 * @return array|bool
	 * @since 1.0.4
	 *
	...
	private function get_data_check_update_plugin() {
		$api_url  = Phys_Admin_Config::get( 'api_check_self_update' );
	...
	/**
	 * Get information plugin phys core.
	 *
	 * @return array|bool
	 * @since 1.0.4
	 *
	...
	private function get_plugin_information_remote() {
		$data        = get_site_transient( 'phys_core_check_self_update' );
		$force_check = ! empty( $_GET['force-check'] );

		if ( empty( $data ) || $force_check ) {
			$data = $this->get_data_check_update_plugin();
			set_site_transient( 'phys_core_check_self_update', $data, 600 );
		}
	...
	private function get_current_version() {
		return defined( 'PHYS_CORE_VERSION' ) ? PHYS_CORE_VERSION : false;
	}
	...
	private function is_support() {
		return ( version_compare( PHYS_CORE_VERSION, '1.0.3', '>' ) );
	}
}

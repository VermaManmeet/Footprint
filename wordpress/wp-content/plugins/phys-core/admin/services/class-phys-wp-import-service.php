...
phys_core_set_time_limit();

require_once PHYS_CORE_ADMIN_PATH . '/includes/wordpress-importer/wordpress-importer.php';

/**
 * Class Phys_WP_Import
 *
 * @since     0.3.1
 * @package   Phys_Core_Admin
 */
class Phys_WP_Import_Service extends WP_Import {
	/**
	 * @since 1.0.6
	 *
	 * @var null
	 ...
	 * @var string
	 */
	protected $_phys_demo_content_key = '_phys_demo_content';
	...
	/**
	 * Get single instance Phys_WP_Import_Service.
	 *
	 * @since 1.0.6
	 *
	 * @param bool $clean
	 *
	 * @return Phys_WP_Import_Service
	 */
	...
	private function allow_edit_variables() {
		$posts_pp       = get_option( 'phys_importer_posts_pp' );
		$attachments_pp = get_option( 'phys_importer_attachments_pp' );
		...
		if ( defined( 'PHYS_IMPORTER_POSTS_PP' ) ) {
			$this->_posts_per_page = PHYS_IMPORTER_POSTS_PP;
		}

		if ( defined( 'PHYS_IMPORTER_ATTACHMENTS_PP' ) ) {
			$this->_attachments_per_page = PHYS_IMPORTER_ATTACHMENTS_PP;
		}
	}
	...
	private function _get_demo_image() {
		...
		$image_path = apply_filters( 'phys_core_importer_path_demo_image', $image_path );
		if ( file_exists( $image_path ) ) {
			$image['path'] = $image_path;
			$image['uri']  = apply_filters( 'phys_core_importer_uri_demo_image', get_stylesheet_directory_uri() . '/inc/data/demo_image.jpg' );

			return $image;
		}

		return array(
			'path' => PHYS_CORE_ADMIN_PATH . '/assets/images/demo_image.png',
			'uri'  => PHYS_CORE_URI . '/admin/assets/images/demo_image.png',
		);
	}
	...
	private function _insert_attachment_demo_image() {
		...
		update_post_meta( $attach_id, '_phys_demo_attachment', 'yes' );
	...
	public function process_categories() {
		...
		foreach ( $this->categories as $cat ) {
			...
			if ( ! is_wp_error( $id ) ) {
				...
				do_action( 'phys_import_processed_term', $id, $cat );
		...
		do_action( 'phys_import_processed_terms', $this->processed_terms, $this->categories );
	...
	private function _import_posts( $posts, $attachment_demo_image_id = false ) {
		...
		foreach ( $this->posts as $post ) {
			do_action( 'phys_import_import_post', $post );
		...
		if ( ! empty( $post['postmeta'] ) ) {
			foreach ( $post['postmeta'] as $meta ) {
				...
				if ( $key ) {
					...
					if ( ! $value ) {
						$value = phys_maybe_unserialize( $meta['value'] );
					}
					...
					if ( $key === '_elementor_data' ) {
						$meta_handler = new Phys_Elementor_Import_Service( $value, $this->_chunks['base_blog_url'] );
	...
	public function analyze_content( $xml ) {
		...
		$this->_chunks = $chunks;
		$this->_set_cache_data( 'chunks', $this->_chunks );
		do_action( 'phys_import_parse_demo_data', $this );
	}
	...
	public function get_current_demo( $field = '' ) {
		$current_demo = get_option( 'phys_importer_current_demo' );
	...
	public function get_demo_cache_path() {
		...
		$this->_cache_dir = $upload_dir['basedir'] . "/phys-import-demo/" . $this->get_current_demo( 'demo' );

		//$this->rmdir( $this->_cache_dir );

		if ( ! Phys_File_Helper::mkdir( $this->_cache_dir ) ) {
			die( sprintf( __( 'Unable to create directory %s' ), $this->_cache_dir ) );
		}

		Phys_File_Helper::mkdir( $this->_cache_dir . '/cache' );
	...
	public function import_object_meta( $return_me, $meta_key, $unused = '' ) {
		if ( '_phys_demo_content' == $return_me ) {
			$return_me = false;
		}
	...
	private function _clean_comments() {
		...
		$query = $wpdb->prepare( "
			...
			WHERE cm.meta_key = %s
			AND cm.meta_value = %s
		", $this->_phys_demo_content_key, 'yes' );
	...
	private function _clean_posts() {
		...
		// Firstly, we need to delete all posts (and it's meta data) has meta $this->_phys_demo_content_key is 'yes'.
		...
		$query    = $wpdb->prepare( "
			...
			AND pm.meta_value = %s
			AND p.post_type <> %s
		", $this->_phys_demo_content_key, 'yes', 'attachment' );
	...
	private function _clean_cache_folder() {
		Phys_File_Helper::rmdir( $this->_cache_dir . '/cache' );
		Phys_File_Helper::mkdir( $this->_cache_dir . '/cache' );
	}

	/**
	 * Mark a post is demo content imported by PhysImporter
	 *
	 ...
	 * @return mixed
	 */
	private function _set_object_is_demo_content( $object_type, $object_id, $key = '_phys_demo_content', $value = 'yes' ) {
	...
	function fetch_remote_file( $url, $post ) {
		...
		$download = Phys_Remote_Helper::download( $url, $upload['file'] );

		if ( is_wp_error( $download ) ) {
			phys_add_log( $download->get_error_message(), $download->get_error_code() );
